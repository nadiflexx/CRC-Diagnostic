...
Entrenamiento: Clasificador de Cáncer de Colon con MLflow.
3 clases (solo colonoscopia): Normal | Pólipo | Inflamación.

Incluye:
- Mixup augmentation (rompe atajos espaciales)
- Temperature scaling post-training (calibra confianza)
- Soporte multi-source (HyperKvasir + CVC-ClinicDB + LIMUC)
"""
...
def mixup_data(x, y, alpha=0.2):
    """
    Mixup: mezcla pares de imágenes y etiquetas.
    Referencia: Zhang et al. 2018 "mixup: Beyond Empirical Risk Minimization"
    """
...
def mixup_criterion(criterion, pred, y_a, y_b, lam):
    """Loss para Mixup: combinación ponderada de ambos targets."""
...
class TemperatureScaler:
    """
    Calibra la confianza del modelo post-entrenamiento.
    ...
    def fit(self, model, val_loader, device):
        """Encuentra la temperatura óptima usando el val set."""
        model.eval()
        all_logits = []
        all_labels = []
        ...
        logger.info(f"  🌡️  Temperatura óptima: {self.temperature:.2f}")

        return self.temperature

    def scale(self, logits):
        """Aplica la temperatura a los logits."""
...
class ImageClassifierTrainer:
    """Trainer para clasificador de cáncer de colon (multi-source)."""
    ...
    def __init__(
        ...
        logger.info(f"  Proyecto: {self.class_mapping.get('project', 'N/A')}")
        logger.info(f"  Clases ({self.num_classes}): {list(self.class_names.values())}")
        logger.info(f"  Fuentes: {self.class_mapping.get('sources', ['hyperkvasir'])}")
    ...
    def _load_class_mapping(self):
        """Busca class_mapping.json en paths multi-source y legacy."""
        for p in [
            # Multi-source paths
            paths.COLON_PROCESSED / "class_mapping.json",
            paths.COLON_CLEAN / "class_mapping.json",
            # Legacy paths (single-source HyperKvasir)
            ...
            if p.exists():
                with open(p) as f:
                    mapping = json.load(f)
                logger.info(f"  📄 class_mapping cargado de: {p}")
                return mapping

        raise FileNotFoundError(
            "class_mapping.json no encontrado. "
            "Ejecuta: organize_multi_dataset.py o organize_hyperkvasir.py"
        )
    ...
    def _verify_no_leakage(self, data):
        ...
        if overlaps:
            raise ValueError(
                f"Data leakage: {len(overlaps)} imágenes compartidas entre splits"
            )
        logger.info("  ✅ Sin data leakage entre splits")

    def _log_distribution(self, data):
        """Log de distribución por clase y fuente."""
        for split in ["train", "val", "test"]:
            ...
            logger.info(f"         fuentes: {source_detail}")
    ...
    def train(
        ...
        """Entrenamiento con Mixup y Temperature Scaling."""

        # Detectar fuentes usadas
        ...
        with self.tracker.start_run(f"{self.model_name}_multisource"):
            ...
            # ── Training loop ──
            logger.info(f"\n═══ ENTRENAMIENTO ({self.num_classes} CLASES) ═══")
            logger.info(f"  Epochs: {epochs} | Batch: {batch_size} | LR: {lr}")
            logger.info(f"  Mixup: {'✅' if use_mixup else '❌'} (alpha={mixup_alpha})")
            logger.info(f"  Clases: {list(self.class_names.values())}")
            logger.info(f"  Fuentes: {sources_str}")
            ...
            for epoch in range(epochs):
                ...
                if epoch == 0:
                    ...
                    if val_metrics["accuracy"] > 0.98:
                        logger.error(
                            "🚨 Accuracy >98% en epoch 1 → Posible Data Leakage"
                        )
                    elif val_metrics["accuracy"] > 0.90:
                        logger.info("  ✅ Normal con ImageNet-21k pretrain")
                    else:
                        logger.info("  ✅ Crecimiento normal")
            ...
            logger.info(f"\n✅ Entrenamiento completado. Mejor F1={self.best_f1:.4f}")
            ...
            # Guardar temperatura en el checkpoint
    ...
    def _evaluate_on_test(self, data, img_size):
        """Evaluación detallada en test set con análisis por fuente."""
        logger.info("\n" + "=" * 70)
        logger.info("TEST SET - EVALUACIÓN FINAL (MULTI-SOURCE)")
        logger.info("=" * 70)
        ...
        if not test_paths:
            logger.warning("No hay datos de test")
            return None
        ...
        logger.info(f"  Temperatura: {self.temp_scaler.temperature:.2f}")
        ...
        if clinical:
            ...
            for label_str, info in clinical.items():
                ...
                logger.info(
                    ...
                    f"Riesgo: {info.get('risk_level', 'N/A')} | "
                    f"Acción: {info.get('action', 'N/A')}"
                )
    ...
    def evaluate_on_test(self):
        """Evaluación standalone en test set."""
        data = self._load_data_from_db()
    ...
    def _save_checkpoint_with_temperature(self, temperature):
        """Actualiza el checkpoint con la temperatura calibrada."""
        path = paths.CLASSIFIER_CHECKPOINT
        if path.exists():
            ...
            logger.info(
                f"  🌡️  Checkpoint actualizado con temperatura={temperature:.2f}"
            )
    ...
    def load_best(self):
        ...
        path = paths.CLASSIFIER_CHECKPOINT
        if not path.exists():
            logger.warning("No se encontró checkpoint")
            return
        ckpt = torch.load(path, map_location=self.device, weights_only=False)
        ...
        logger.info(
            f"✅ Cargado: F1={ckpt.get('best_f1', 0):.4f} "
...
if __name__ == "__main__":
    logger.info("=" * 70)
    logger.info("CLASIFICADOR DE CÁNCER DE COLON (MULTI-SOURCE)")
    logger.info("  Clases: Normal | Pólipo | Inflamación")
    logger.info("  Fuentes: HyperKvasir + CVC-ClinicDB + LIMUC")
