...
class ReverseLogicAnalyzer:
    ...
    def plot_roc_curve(
        ...
        plt.plot(
            [0, 1], [0, 1], linestyle="--", color="#94a3b8", linewidth=2, label="Random"
        )
        plt.xlabel("False Positive Rate", fontsize=12, fontweight="bold")
        plt.ylabel("True Positive Rate", fontsize=12, fontweight="bold")
        plt.title(
            f"ROC Curves - {target_name} Prediction", fontsize=14, fontweight="bold"
        )
    ...
    def plot_confusion_matrices(
        ...
        if y_test.dtype == "object":
            y_test_binary = (y_test == "Yes").astype(int).to_numpy()
        else:
        ...
        plt.suptitle(
            f"Confusion Matrices - {target_name}",
            fontsize=14,
            fontweight="bold",
            y=1.02,
        )
    ...
    def plot_confusion_matrix_detailed(
        ...
        for i, metric in enumerate(metrics_to_plot):
            ax.bar(
                x + i * width, metrics_df[metric], width, label=metric, color=colors[i]
            )

        ax.set_xlabel("Model", fontweight="bold", fontsize=12)
        ax.set_ylabel("Score (%)", fontweight="bold", fontsize=12)
        ax.set_title(
            f"Model Performance Metrics - {target_name}", fontweight="bold", fontsize=14
        )
    ...
    def plot_feature_importance(
        ...
        plt.suptitle(
            f"Feature Importance - {target_name}", fontsize=14, fontweight="bold"
        )
        plt.tight_layout()
        plt.savefig(self.output_dir / filename, dpi=150, bbox_inches="tight")
        plt.close()
    ...
    def plot_prediction_histograms(
        ...
        for model_name, y_prob in predictions.items():
            ...
            ax_top.set_yticklabels(["True No", "True Yes"])
            ...
            ax_hist.axvline(
                median_no, color="#475569", linestyle="--", linewidth=1.5, alpha=0.9
            )
            ax_hist.axvline(
                median_yes, color="#1d4ed8", linestyle="--", linewidth=1.5, alpha=0.9
            )
            ...
            output_path = (
                self.output_dir
                / f"{filename_prefix}_{safe_model_name}_{target_name.lower()}.png"
            )
    ...
    def run_full_analysis(
        ...
        predictions_proba = {mt: model.predict_risk(X_test, mt) for mt in model_types}
        predictions_binary = {mt: model.predict(X_test, mt) for mt in model_types}
        ...
        if "Country" in full_df.columns:
            ...
            print("Skipping country plot: 'Country' not in dataset.")
