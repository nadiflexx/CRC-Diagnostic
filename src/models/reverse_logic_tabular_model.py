...
class FeatureWeightedXGBClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(
        ...
        feature_weight_overrides: dict[str, float] | None = None,
    ):
...
class ReverseLogicTabularModel:
    ...
    def _encode_target(
        ...
        if isinstance(y.iloc[0], str):
            if fit_encoder or model_type not in self.label_encoders:
                ...
                return encoded
            return pd.Series(
                self.label_encoders[model_type].transform(y), index=y.index
            )
    ...
    def fit(
        ...
        if model_type == "xgboost":
            ...
            raise ValueError(
                f"Unknown model_type: {model_type}. Supported: xgboost, random_forest, lightgbm"
            )
    ...
    def cross_validate_model(
        ...
        if model_type == "xgboost":
            ...
            raise ValueError(
                f"Unknown model_type: {model_type}. Supported: xgboost, random_forest"
            )
        ...
        summary: dict[str, Any] = {"cv_folds": cv_folds}
    ...
    def predict(
        ...
        if model_type not in self.pipelines:
            raise ValueError(
                f"Model {model_type} not trained. Available: {list(self.pipelines.keys())}"
            )
    ...
    def evaluate(
        ...
        """
        y_test_encoded = self._encode_target(
            y_test, model_type, fit_encoder=False
        ).values
        ...
        unique_classes = np.unique(np.asarray(y_test_encoded))
        roc_auc = (
