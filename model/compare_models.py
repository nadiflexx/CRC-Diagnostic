...
Benchmark de 5 clasificadores sobre el dataset clínico-tumoral de CRC.
Objetivo: justificar empíricamente la elección de XGBoost frente a alternativas
más simples o complejas.

Métricas reportadas por modelo (media ± std con CV 5-fold estratificado):
...
def evaluar_modelos(X: pd.DataFrame, y: pd.Series, modelos: dict, n_splits: int = 5) -> pd.DataFrame:
    ...
    for nombre, modelo in modelos.items():
        ...
        resultados = cross_validate(
            ...
            cv=cv,
            scoring=SCORERS,
            n_jobs=1,         # evita conflictos con los n_jobs internos de cada modelo
        ...
        # Guardamos la media de AUC para poder ordenar la tabla
...
def imprimir_tabla(df_res: pd.DataFrame) -> None:
    ...
    for rank, row in df_res.iterrows():
        # Subrayamos la fila de XGBoost con un marcador visual
    ...
    print()
    print("  Nota: Recall = Sensibilidad (TPR) | Specificity = TNR = TNR (TN / TN+FP)")
